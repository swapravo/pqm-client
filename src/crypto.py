...
def insert_private_key(key, keyname):
	out, err = src.utils.execute("./src/ccr -y -I --name " + keyname, key)
	if err or out:
		print("Private key insertion FAILED!")
		print(out, err)
		return 1
	return 0
...
def remove_public_key(keyname):
	...
	if err or out:
		print("Public key removal FAILED!")
		print(out, err)
		return 1
	return 0
...
def remove_private_key(keyname):
	out, err = src.utils.execute("./src/ccr -y -X " + keyname)
	if err or out:
		print("Private key removal FAILED!")
		print(out, err)
		return 1
	return 0
...
def key_fingerprint(keyname):
	...
	if err:
		print("Key fingerprinting FAILED!")
		print(out, err)
		return 1
...
def asymmetrically_encrypt(message, public_key_name):
	...
	if not out or err:
		print("Asymmetric encryption FAILED!")
		print(out, err)
		return 1
	return out
